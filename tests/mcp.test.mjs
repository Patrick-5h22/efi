...
test('mcp : une recherche renvoie des options et un texte lisible', () => {
  const r = chercherCreneaux(base(), { formations: CATS, type: 'Initial', a_partir_du: '2026-09-15' });
...
test('mcp : la recherche respecte la date demandée', () => {
  const r = chercherCreneaux(base(), { formations: CATS, a_partir_du: '2026-09-16' });
...
test('mcp : nb_options est borné', () => {
  const r = chercherCreneaux(base(), { formations: CATS, nb_options: 99 });
...
test('mcp : une recherche ne modifie pas l’état reçu', () => {
  ...
  chercherCreneaux(state, { formations: CATS });
...
test('mcp : aucune possibilité → message explicite, pas une erreur', () => {
  const r = chercherCreneaux(base({ jours: [] }), { formations: CATS });
  assert.deepEqual(r.options, []);
  assert.match(r.texte, /Aucune possibilité/);
});

test('mcp : catégorie inconnue → erreur métier nommant le catalogue', () => {
  assert.throws(() => chercherCreneaux(base(), { formations: ['R489-42'] }), (e) => {
...
test('mcp : aucune catégorie → erreur métier', () => {
  assert.throws(() => chercherCreneaux(base(), { formations: [] }), (e) => e.metier);
  assert.throws(() => chercherCreneaux(base(), {}), (e) => e.metier);
});
...
test('mcp : la pré-réservation pose des lignes « pré-réservée »', () => {
  const state = base();
  const jour = chercherCreneaux(state, { formations: CATS, a_partir_du: '2026-09-15' }).options[0].jour;
...
test('mcp : l’état rendu est sain et contient les nouvelles lignes', () => {
  const state = base();
  const jour = chercherCreneaux(state, { formations: CATS, a_partir_du: '2026-09-15' }).options[0].jour;
  const r = preReserver(state, { stagiaire: 'DURAND Thomas', formations: CATS, jour });
...
test('mcp : la pré-réservation ne modifie pas l’état reçu', () => {
  ...
  const jour = chercherCreneaux(state, { formations: CATS, a_partir_du: '2026-09-15' }).options[0].jour;
  const avant = JSON.stringify(state);
  preReserver(state, { stagiaire: 'DURAND Thomas', formations: CATS, jour });
...
test('mcp : le nom du stagiaire est obligatoire pour réserver', () => {
  const state = base();
  assert.throws(() => preReserver(state, { formations: CATS, jour: '2026-09-15' }), (e) => {
    assert.ok(e.metier);
...
test('mcp : un jour absent ou mal formé est refusé', () => {
  ...
  for (const jour of [undefined, '', '15/09/2026', 'demain']) {
    assert.throws(() => preReserver(state, { stagiaire: 'X', formations: CATS, jour }), (e) => e.metier);
  }
});
...
test('mcp : deux pré-réservations successives ne se chevauchent pas', () => {
  const state = base();
  const j1 = chercherCreneaux(state, { formations: CATS, a_partir_du: '2026-09-14' }).options[0].jour;
  const un = preReserver(state, { stagiaire: 'PREMIER Candidat', formations: CATS, jour: j1 });

  const j2 = chercherCreneaux(un.state, { formations: CATS, a_partir_du: j1 }).options[0].jour;
  const deux = preReserver(un.state, { stagiaire: 'SECOND Candidat', formations: CATS, jour: j2 });
...
test('mcp : dans une option, la date de début n’est pas annoncée deux fois', () => {
  ...
  const r = chercherCreneaux(base(), { formations: CATS, a_partir_du: '2026-09-14', nb_options: 2 });
